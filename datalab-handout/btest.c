 ...
 * 
 * Note: 64-bit safe. The puzzles operate on 32-bit int/unsigned (the same
 * width under both the ILP32 and LP64 ABIs) and this harness stores all test
 * values and results in int/unsigned/double with no pointer-to-int casts, so
 * it builds and grades identically under gcc -m64 (the default) and -m32.
 */
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
