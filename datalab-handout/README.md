...
BDD checker: The BDD checker exhaustively verifies the correctness of
your INTEGER puzzle solutions. After you've debugged a puzzle using
btest, use the BDD checker as the definitive correctness test for the
integer puzzles; if it finds an error, it reports the argument value(s)
that gave the wrong answer. The floating-point puzzles are checked by
btest, not the BDD checker.
...
driver.pl: This is a driver program that checks your solutions in bits.c
for correctness and adherence to the programming guidelines. It uses dlc
for the coding rules, the BDD checker for the integer puzzles, and btest
(which it compiles with gcc/make) for the floating-point puzzles. It also displays your total correctness
...
driver.pl*	- Driver: dlc + BDD checker (integer puzzles) + btest (float puzzles); same program Autolab uses to autograde bits.c
